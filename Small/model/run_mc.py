...
# Also add the parent directory to allow importing from the main project folder
...
import run_test
from model.data_generator import get_dataset_directory_structure, generate_real_data_id
...
def main(dichotomy_info, level, study_mode):
    ...
    # The config path is now relative to this script's location (the model directory)
    config_path = os.path.join(script_dir, "config.yaml")
    
    # --- DEBUGGING ---
    logger.info(f"Attempting to load config from absolute path: {os.path.abspath(config_path)}")
    # --- END DEBUGGING ---
    
    # Load config to check if it exists before proceeding
    try:
        with open(config_path, 'r') as f:
            yaml.safe_load(f)
    except Exception as e:
