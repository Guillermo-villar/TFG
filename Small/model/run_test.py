...
def load_modules():
    ...
    # Import data_generator from the model package
    try:
    ...
    # Import test_LSEnsemble from the model package
    try:
        spec = importlib.util.spec_from_file_location(
            "test_LSEnsemble", 
...
def create_run_directory_for_dataset(dataset_id, base_dir=None):
    ...
    # Import data_generator module from the model package
    script_dir = os.path.dirname(os.path.abspath(__file__))
...
def main(level=None, study_mode="full_study", output_dir=None):
    ...
    # Get script directory, which is the model directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    # The config file is now inside the model directory
