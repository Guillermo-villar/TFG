...
def create_dataloader(X, y, weights, batch_size, mode='random'):
    ...
    if mode == 'random':
    ...
    elif mode == 'class_equitative' or mode == 'representative':
        ...
        y_int = y_mapped.to(torch.int64).numpy()  # Convert to integers for calculations
        ...
        # Ensure batch sizes are correctly calculated
        num_samples = len(y)
        num_batches = int(np.ceil(num_samples / batch_size))  # Number of batches

        # Handle the representative mode logic
        if mode == 'representative':
            class_labels, samples_class = np.unique(y_int, return_counts=True)
            batch_samples_class = []
            list_samples_class = []

            for k in range(len(class_labels)):
                batch_samples_class.append(int(np.ceil(batch_size * samples_class[k] / num_samples)))
                mod = (batch_samples_class[k] * num_batches) // samples_class[k]
                rem = (batch_samples_class[k] * num_batches) % samples_class[k]
                ind_class = np.where(y_int == class_labels[k])[0]
                list_aux = list(ind_class) * mod + list(np.random.choice(ind_class, rem))
                np.random.shuffle(list_aux)
                list_samples_class.append(list_aux)

            list_samples_batch = []
            for kbatch in range(num_batches):
                list_aux = []
                for kclass in range(len(class_labels)):
            ...
            batch_indices = np.concatenate(list_samples_batch)
            return DataLoader(TensorDataset(X[batch_indices], y[batch_indices], weights[batch_indices]), batch_size=batch_size)

        else:
            # Default WeightedRandomSampler for 'class_equitative' mode
