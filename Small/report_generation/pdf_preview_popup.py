...
if __name__ == "__main__":
    ...
    def parse_experiment_folder(experiment_folder):
        ...
        dichotomies = [d for d in os.listdir(experiment_folder) 
            ...
            if os.path.exists(progress_file):
                try:
                    with open(progress_file, 'r') as f:
                        ...
                        # Create dichotomy mapping for OVO strategy
                        if strategy == 'ovo' and 'class_names' in progress_data:
                            ...
                            for i in range(len(class_names)):
                                for j in range(i + 1, len(class_names)):
                                    ...
                                    dichotomy_counter += 1
                        elif strategy == 'ova' and 'class_names' in progress_data:
                            class_names = progress_data['class_names']
        ...
        for d in dichotomies:
            ...
            if os.path.isdir(runs_dir):
                run_folders = sorted([r for r in os.listdir(runs_dir) 
                                    if os.path.isdir(os.path.join(runs_dir, r))], reverse=True)
    ...
    if len(sys.argv) > 1:
        ...
        experiment_path = "/home/ubuntu/Downloads/Code/NeonGenesis/Small/datasets/IDs/real_903626d0"
    
    print(f"Analyzing experiment: {experiment_path}")
    
    # Parse experiment data
    experiment_data = parse_experiment_folder(experiment_path)
    print(f"Experiment type: {'Multiclass' if experiment_data['is_multiclass'] else 'Binary'}")
    print(f"Classes: {experiment_data['class_distribution']}")
    
    # Generate HTML report
    html_generator = ReportHTMLGenerator(experiment_data)
    html_content = html_generator.render_html()
    
    # Generate PDF
    pdf_generator = PDFReportGenerator(html_content)
    temp_pdf_path = pdf_generator.save_temp_pdf()
    print(f"PDF generated: {temp_pdf_path}")
    
    # Show preview
    root = tk.Tk()
    root.withdraw()  # Hide root window
    
    def save_final_pdf(file_path):
        pdf_generator.save_pdf(file_path)
    
    PDFPreviewPopup(root, temp_pdf_path, save_callback=save_final_pdf)
    root.mainloop()
