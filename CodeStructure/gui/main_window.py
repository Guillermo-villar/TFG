...
class MLExperimentGUI:
        ...
        if filepath:
            ...
            try:
                ...
                if multiclass_candidates:
                    ...
                    selected_strategy = self.popup_manager.show_multiclass_confirmation_popup(main_candidate)
    ...
    def get_current_csv_path(self, for_report_generation=False):
        ...
        for_report_generation : bool
            If True, returns the original CSV path for report generation purposes.
        """
        # When generating a report, we prefer the original unprocessed file path
        if for_report_generation and self.original_csv_path:
            return self.original_csv_path

        # For regular experiment runs, use the processed path if available
        data_source = self.data_source_var.get()
        if data_source == "real":
            if self.user_csv_path:
    ...
    def on_closing(self):
        """Handle window close"""
        if self.is_running:
            if messagebox.askokcancel(self.texts["quit"], self.texts["quit_confirmation"]):
