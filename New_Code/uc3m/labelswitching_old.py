...
def create_dataloader(X, y, weights, batch_size, mode='random'):
    ...
    - y: Labels (torch.Tensor) in -1,+1 format.
    ...
    if mode == 'random':
        ...
        # Map -1, +1 to 0, 1 for internal calculations
        y_mapped = (y + 1) // 2  # -1 -> 0, +1 -> 1

        # Convert `y_mapped` to integers if needed
        y_int = y_mapped.to(torch.int64).numpy()

        # Calculate class counts
        class_counts = np.bincount(y_int)
        if np.any(class_counts == 0):
            raise ValueError("At least one class in `y` has no samples.")

        # Calculate sample weights
        class_weights = 1.0 / class_counts
        sample_weights = class_weights[y_int]

        # Use WeightedRandomSampler with sample weights
...
class AsymmetricMLP1(BaseAsymmetricMLP):
    def __init__(self, input_size, hidden_size, alpha, beta, dropout_prob=0.0):
        super().__init__(input_size, hidden_size, alpha, beta)
    ...
    def forward(self, x):
        ...
        o = torch.tanh(self.hidden0(x))  # Apply activation to hidden layer
...
class LSEnsemble(nn.Module):
    ...
    def initialize_experts(self, input_size):
        ...
        self.experts = nn.ModuleList([AsymmetricMLP1(input_size, self.hidden_size, self.alpha, self.beta, self.drop_out) 
    ...
    def generate_experts_data(self, x, y, w=None, Q_RB_S=1, RB_each_expert=True):
        ...
        N0_tr, N1_tr = np.sum(y_np == -1), np.sum(y_np == 1)
        P0_tr, P1_tr = N0_tr / (N0_tr + N1_tr), N1_tr / (N0_tr + N1_tr)
        QP_tr = P0_tr / P1_tr
        self.QP_tr = QP_tr
...
class LSEnsembleWrapper(BaseEstimator):
    def __init__(self, hidden_size, num_experts, alpha, beta, Q_RB_C, Q_RB_S=1, 
                 n_epoch=1, n_batch=1, lbfgs=True, mode='random'):
        """
        Wrapper for the LSEnsemble model to allow use in scikit-learn-style interfaces.
        
        Parameters:
        - hidden_size: Number of neurons in the hidden layer for each expert.
        - num_experts: Number of experts in the ensemble.
        - alpha: Regularization parameter for the ensemble.
        - beta: Regularization parameter for the experts.
        - Q_RB_C: Coefficient for expert weights computation.
        - Q_RB_S: Scaling factor for expert weights.
        - n_epoch: Number of training epochs.
        - n_batch: Batch size for training.
        - lbfgs: Whether to use LBFGS optimizer.
        - mode: Sampling mode ('random', 'class_equitative', etc.).
        """
        self.hidden_size = hidden_size
        self.num_experts = num_experts
        self.alpha = alpha
